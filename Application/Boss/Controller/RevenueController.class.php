...
class RevenueController extends AuthController {
    ...
    public function searching() {
        ...
        if (!IS_GET) {
            ...
            $Page = new Page($count, $everypage); // 实例化分页类 传入总记录数和每页显示的记录数(25)
            $show = $Page->show(); // 分页显示输出// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
            $list = $tbName->field("sum(amount) as total,num,barcode,product,letter")->where($where)->order("sum(amount) desc")->limit($Page->firstRow . ',' . $Page->listRows)->group("product")->select();
            ...
            $shopnumSQL = $tbName->field('sum(num) as shopnum,Goods.letter')->where($where)->order("sum(amount) desc")->group("barcode")->select(FALSE);
