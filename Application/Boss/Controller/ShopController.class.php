...
class ShopController extends AuthController {
    ...
    public function add() {
        ...
        if (!IS_POST) {
            ...
            if ($res) {
                //  跳转之后 再调用 Index/index
                $this->success("添加店铺成功 ！！", U('Shop/index'), 3);
                $this->success("<script language=JavaScript> top.location.href='" . U('Index/index') . "'; </script>");
            } else {
                $this->error("添加店铺失败 ？？");
            }
        }
    ...
    public function delete() {
        ...
        if (empty($comid)) {
            ...
            if ($res) {
                $this->success("删除店铺成功！！", U('Shop/index'));
                $this->success("<script language=JavaScript> top.location.href='" . U('Index/index') . "'; </script>");
            } else {
                $this->error("删除店铺失败！", U('Shop/index'));
            }
        }
    }
