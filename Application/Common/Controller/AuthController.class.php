...
class AuthController extends Controller {
    /*
      ...
      // 判断终端登录类型  PC、移动端   $_SESSION['uid_name_terminal']=$uid."_".$name."_".$terminal;
      // 单一登录(生命周期，每一次操作都要初始化周期值) $_SESSION['action_timeout']=$time;
      }
      }
     * 
     */
    ...
    public function showPage($tbName, $everypage = "10", $where = "", $order = "id desc", $group = "") {
         ...
         */
        $list = $tbName->where($where)->group($group)->order($order)->limit($Page->firstRow . ',' . $Page->listRows)->select();
