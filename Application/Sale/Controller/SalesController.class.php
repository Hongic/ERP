...
// exit();
    ...
    public function tuiAjax() {
        ...
        if ($_POST['code'] != "") {
            ...
            $bar = $presell->where($pre)->order("id desc")->limit(1)->select();
            // var_dump($bar);
            if ($bar == "") {
                $this->error("没有该商品", U('Sales/tui'));
            }
            $this->assign("tui", $bar);
    ...
    public function tuiShows() {
        if (IS_POST) {
            ...
            if ($barcode == "") {
                ...
                $salenum = $_POST['num']; // 数量
                // 删除sales表的记录 bar id desc
                $sales_cc = D($shop_id . "Sales");
                $where_sales['barcode'] = $barcode;
                $res_sales = $sales_cc->field("id,out_id,num")->where("id =$sid")->order("id desc")->find();
                if (!$res_sales) {
                    $this->error("销售表中没有该商品");
                    exit();
                }

                $where_sales["outamount"] = array("gt", 0);   // 库存 >0
                $res_storage = $out->field("id,outamount")->where("id=" . $res_sales['out_id'])->order("id asc")->find();

                $data['outamount'] = $res_storage['outamount'] + $res_sales['num'];
                $whereID['id'] = $res_storage['id'];

                $res = $out->where($whereID)->data($data)->save();
                if (!$res) {
                    $this->error("退货不能存入库存");
                } else {
                    $sales_cc->where(" id =$sid")->order("id desc")->delete(); // 删除该商品的最新销售记录
                    $this->success("退货成功", U('Sales/tui'));
                }
            }
        }
    }
